		...
		<div id="pageintro" class="hoc clear">
			<article>
				<div>
					<h2>Coupling</h2>
					<br>
					<p>Ccp = Nr + Nmcms + Nmcmd + Nmcrms + Nmcrmd + Nrmcrms +
						Nrmcrmd + Nrmcms + Nrmcmd + Nmrgvs + Nmgvd + Nrmrgvs + Nrmrgvd</p>
				</div>
				<footer>
							...
							<form action="coupling.jsp" method="post" enctype="multipart/form-data">
								...
								<table id="sizetable2">
									...
									<tr>
										...
										<td><input type="number" value="2" name="0" ></td>
									</tr>

									<tr>
										<td>A regular method calling another regular method in
											the same file</td>
										<td><input type="number" value="2" name="1" ></td>
									</tr>
									<tr>
										<td>A regular method calling another regular method in a
											different file</td>
										<td><input value="3" type="number" name="2" /></td>
									</tr>
									<tr>
										<td>A regular method calling a recursive method in the
											same file</td>
										<td><input value="3" type="number" name="3" /></td>
									</tr>
									<tr>
										<td>A regular method calling a recursive method in a
											different file</td>
										<td><input value="4" type="number" name="4" /></td>
									</tr>
									<tr>
										<td>A recursive method calling another recursive method
											in the same file</td>
										<td><input value="4" type="number" name="5" /></td>
									</tr>
									<tr>
										<td>A recursive method calling another recursive method
											in a different file</td>
										<td><input value="5" type="number" name="6" /></td>
									</tr>
									<tr>
										<td>A recursive method calling a regular method in the
											same file</td>
										<td><input value="3" type="number" name="7" /></td>
									</tr>
									<tr>
										<td>A recursive method calling a regular method in a
											different file</td>
										<td><input value="4" type="number" name="8" /></td>
									</tr>
									<tr>
										<td>A regular method referencing a global variable in the
											same file</td>
										<td><input value="1" type="number" name="9" /></td>
									</tr>
									<tr>
										<td>A regular method referencing a global variable in a
											different file</td>
										<td><input value="2" type="number" name="10" /></td>
									</tr>
									<tr>
										<td>A recursive method referencing a global variable in
											the same file</td>
										<td><input value="1" type="number" name="11" /></td>
									</tr>
									<tr>
										<td>A recursive method referencing a global variable in a
											different file</td>
										<td><input value="2" type="number" name="12" /></td>
									</tr>
								</table>
						</tr>
					</table>
							...
							try {
								...
								for(FileItem item: fileItems){
									...
									if (item.isFormField()) {
										...
										defaultWeight.set(Integer.parseInt(name), Integer.parseInt(value));
									...
									if (!fi.isFormField()) {
										...
										file = new File("D:\\" + fileName.substring(fileName.indexOf("\\") + 1));
										fi.write(file);
										System.out
												.println(fileName.substring(fileName.indexOf("\\") + 1) + "  file we dealing with");
										fileList.add(file);
								...
								fileList = fileList.stream().filter(e -> e.getName().endsWith(".java"))
								...
								for (File nowfile : fileList) {
									...
									while (m.find()) {
										...
										String onlyMethodName = methodWithAccessAndReturn
												.substring(methodWithAccessAndReturn.lastIndexOf(" "));
					...
					<%
						//size tabel ende
									...
									thisFileMethods.entrySet().stream().forEach((entry) -> {
										...
										thisFileMethods.entrySet().stream().filter(e -> !e.getKey().equals(entry.getKey()))
												.forEach(methodName -> {
													...
													if (matcher.find()) {
														System.out.println(
																entry.getKey() + " method  inner call " + methodName.getKey());
									...
									//variable complexity tabel started
					%>
							...
							<%
								//validate size table
											...
											for (int i1 = 0; i1 < 0; i1++) {
												...
												Map<String, String> keys = allGlobalVar.entrySet().stream()
														.filter(e -> e.getKey().contains(stupidFinal))
												...
												if (keys.containsKey(number.trim() + "," + currentClassName)
														&& !codeLine[0].contains("class")) {
													...
													if (codeLine[0].matches(".+(byte|short|int|long|float|double|boolean|char).+")) {
														...
														//System.out.println(originalCodeLine + "  this is not primitive and global");
														global = 1;
														scopeType = 2;
													}
													...
													if (codeLine[0].trim().matches("(.+)=.+;")) {
														if (codeLine[0].matches(".+(byte|short|int|long|float|double|boolean|char).+")) {
															...
															//System.out.println(originalCodeLine + "  this is not primitive and not global");
															global = 1;
															scopeType = 1;
														}
													}
						...
						<tbody>
							...
							<%
								for (int i1 = 0; i1 < 0; i1++) {
												...
												Matcher methoFound = Pattern.compile("(\\w+)( )+(\\w+)+( )*\\((.*)\\)( )*\\{")
					...
					<table style="width: 100%">
						...
						<tbody>
								...
								for (int i1 = 0; i1 < list.size(); i1++) {
												...
												allGlobalVar.entrySet().stream().forEach(e -> {
													if (codeLine[0].contains(e.getKey().split(",")[1].trim() + "." + e.getValue())) {
														...
														codeLine[0] = (codeLine[0]
																.replaceAll(e.getKey().split(",")[1].trim() + "." + e.getValue(), ""));
												...
												thisFileMethods.values().stream().filter(e -> e.isRecursiveCall())
							...
							<tr>
								...
								<%
									table5.put(number, ((isRecursiveMethod[0]) ? defaultWeight.get(0) : 0)
															+ normalToNormalVal[0] * defaultWeight.get(1)
															+ normalToRecursiveVal[0] * defaultWeight.get(3)
															+ RecursiveToRecursiveVal[0] * defaultWeight.get(5)
															+ RecursiveToNormalVal[0] * defaultWeight.get(7)
															+ globalUsedByNonR * defaultWeight.get(9) + globalUsedByR * defaultWeight.get(11));
								%>
								<td><%=((isRecursiveMethod[0]) ? defaultWeight.get(0) : 0)
									+ normalToNormalVal[0] * defaultWeight.get(1)
									+ normalToRecursiveVal[0] * defaultWeight.get(3)
									+ RecursiveToRecursiveVal[0] * defaultWeight.get(5)
									+ RecursiveToNormalVal[0] * defaultWeight.get(7)
									+ globalUsedByNonR * defaultWeight.get(9) + globalUsedByR * defaultWeight.get(11)%></td>
											...
											for (int i1 = 0; i1 < 0; i1++) {
												...
												while (matchAgain.find()) {
													...
													if (originalCodeLine.contains("extends")) {
														...
														if (extendedClass.find()) {
															// 					fullClassMap += classesAndData.get(extendedClass.group(2).trim());
															// 					System.out.println(fullClassMap + "  data added "
					...
					<table hidden style="width: 100%">
						...
						<tbody>
							...
							<tr>
								...
								<td><%=table1.get(number) + table2.get(number) + table3.get(number) + table4.get(number)
									+ table5.get(number) + table6.get(number)%></td>
						...
						<footer id="footer" class="hoc clear">
							<div class="one_half first">
								...
								<p class="btmspace-30">
									...
									the very best for customer needs.<br>Join with us & get
									better resolution.
								</p>
							</div>
